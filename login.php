...
<body id="login">
    ...
    <script type="module">
        ...
        // Sesion cookie en soporte
        let supportIframe = document.createElement('iframe')
        supportIframe.name = 'supportLogin'; supportIframe.style.display = 'none'
        supportIframe.src = 'https://soporte.comunicacionvisualcanarias.com/login.html'
        document.body.appendChild(supportIframe)
        ...
        function login(e) {
            ...
            if ( $('loginForm').checkValidity() ) {
                ...
                fetch('/api/login.php', {method: 'POST', body: loginForm}).then(resp => resp.json()).then( (data)=> {
                    if (data.status == 'ok') { 
                        let loginData = new FormData()
                        loginData.append('user','omar'); loginData.append('pass','1')

                        fetch('https://soporte.comunicacionvisualcanarias.com/gestion.html', {method: 'POST', mode: 'no-cors', body: loginData, credentials: 'include'})
